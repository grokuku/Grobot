#### Ce fichier sert de référence unique et doit être fourni en intégralité au début de chaque session.
    ...
    *   **Implémentation de l'Apprentissage Continu (ACE - Phase 1)**
        *   **Analyse :** Conformément au plan d'intégration `ace_integration.md`, la première phase de l'intégration du framework ACE a été mise en œuvre pour doter les bots d'une capacité d'apprentissage. L'objectif était de commencer à construire une mémoire stratégique ("Playbook") pour chaque bot de manière non-invasive, en mode "observateur", sans affecter leur comportement actuel.
        *   **Résolution :** L'implémentation a été réalisée sur l'ensemble de la pile de services backend. **1. Dépendances :** Les librairies `ace-framework` (v0.2.0) et sa dépendance transitive requise `litellm` ont été ajoutées à `requirements.txt`. **2. Infrastructure :** Un volume Docker persistant a été configuré dans `docker-compose.yml` pour stocker les fichiers de Playbook (`./data/playbooks`). **3. Worker Asynchrone :** Une nouvelle tâche Celery `learn_from_interaction` a été créée dans `app/worker/tasks.py`. Elle contient la logique du cycle d'apprentissage ACE : elle charge le Playbook du bot, instancie les rôles `Reflector` et `Curator` avec un client LLM configuré, utilise un `SelfReflectionEnvironment` pour analyser le contexte de la conversation, et enfin applique les leçons apprises et sauvegarde le Playbook mis à jour. **4. Déclenchement API :** Le endpoint de streaming de chat (`/api/stream/{message_id}` dans `app/api/chat_api.py`) a été modifié pour déclencher de manière asynchrone la tâche `learn_from_interaction.delay(...)` à la fin de chaque interaction réussie.
        *   **Statut :** IMPLÉMENTÉ.
    ...
    *   **Tâche Prioritaire : Implémenter la Phase 2 de l'Intégration ACE (Enrichissement du Contexte).**
        *   **Description :** Maintenant que la Phase 1 (apprentissage en "mode observateur") est fonctionnelle et que les Playbooks sont générés, la prochaine étape est de les utiliser. La tâche consistera à modifier l'orchestrateur d'agents pour charger le Playbook d'un bot et injecter son contenu (`playbook.as_prompt()`) dans le contexte système des agents `Planner` et `Synthesizer` afin d'améliorer la pertinence et la qualité de leurs décisions et réponses.
    ...
    Cette section documente les librairies ou frameworks externes qui jouent un rôle central dans l'architecture, en dehors des standards comme FastAPI ou SQLAlchemy.
    
    *   **Agentic Context Engine (ACE)**
        *   **Nom du Paquet PyPI :** `ace-framework`
        *   **Version lors de l'intégration :** 0.2.0
        *   **Rôle :** Fournit le cœur de la logique d'apprentissage et d'amélioration continue pour les bots. Gère le cycle de vie des "Playbooks" (mémoire stratégique) via les rôles `Reflector`, `Curator`, et `Generator`.
        *   **Installation :** `pip install ace-framework` (gérée via `requirements.txt`).
        *   **Notes :** Traitée comme une "boîte noire" experte. Son intégration est incrémentale et suit le plan défini dans `ace_integration.md`.
    
    *   **LiteLLM**
        *   **Nom du Paquet PyPI :** `litellm`
        *   **Version lors de l'intégration :** (Installée comme dépendance)
        *   **Rôle :** Agit comme une couche de traduction universelle pour les appels aux modèles de langage. Elle est utilisée par `ace-framework` pour communiquer avec notre serveur Ollama via une interface standardisée.
        *   **Installation :** `pip install litellm` (gérée via `requirements.txt`).
        *   **Notes :** Ajoutée manuellement à `requirements.txt` car elle est une dépendance transitive non déclarée de `ace-framework` v0.2.0.