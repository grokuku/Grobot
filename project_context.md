# Project Context: GroBot
    ...
    *   **Migration DB Robuste (Blue/Green) :** Remplacement d'Alembic par `app/database/migration.py`.
    *   **Support Multi-LLM Catégorisé :** Configuration LLM segmentée (Decisional, Tools, Output Client).
    *   **Backend LLM Robuste :** `llm_manager.py` modernisé avec support Async, Streaming résilient, et gestion intelligente de la fenêtre de contexte (Input vs Output).
    *   **Identification d'Outils Fiabilisée (Mécanisme) :** Prompt simplifié pour le `Tool Identifier` et injection dynamique de contexte.

    ### 7.3. Plan d'Action (Priorité Immédiate : Finalisation)

    1.  **Logs UI (Affichage manquant) [A FAIRE] :** Les logs sont visibles côté serveur mais ne remontent pas dans l'interface Web via WebSocket. Il faut vérifier la configuration du `logging` dans `main.py` et le handler WebSocket.
    2.  **Workflows :** Une fois la chaîne d'agents et les logs stabilisés, validation de l'éditeur de workflows.
