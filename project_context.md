...
*   **Crash de la Commande `/prompt_generator` et Amélioration de l'Affichage**
    *   **Analyse :** Le crash initial (`AttributeError`) était le symptôme d'une cascade de défaillances. La cause racine était une réponse JSON-RPC non conforme générée par le serveur MCP (`mcp_routes.py`), qui incluait à la fois une clé `result` et une clé `error: null`, violant la spécification. Cette réponse invalide était ensuite propagée par le proxy API (`tools_api.py`) et faisait crasher le client Discord (`event_handler.py`).
    *   **Résolution :** Un correctif à plusieurs niveaux a été appliqué. **1. Correction de la Cause Racine :** La sérialisation des réponses dans `mcp_routes.py` a été corrigée pour garantir des réponses JSON-RPC conformes. **2. Robustesse de la Chaîne :** Le proxy et le client ont été blindés pour mieux gérer les erreurs et les types de données. **3. Amélioration Fonctionnelle :** Le format de sortie de la commande a été revu. Elle envoie désormais les prompts positif et négatif dans des messages Discord distincts, sous forme de texte en bloc de code (pour faciliter le copier-coller), avec un repli vers un envoi de fichier `.txt` si la limite de caractères est dépassée.
    *   **Statut :** RÉSOLU.

*   **Erreurs de Communication avec Certains Serveurs MCP**
    *   **Analyse :** Une erreur de validation de paramètres survenait lors de l'appel de l'outil `describe_image` via le menu contextuel de Discord. L'enquête a révélé que le client Discord (`discord_bot_launcher/client/event_handler.py`) utilisait des valeurs de paramètres (`'prompt'`, `'english'`) codées en dur qui n'étaient pas conformes au schéma de l'outil MCP.
    *   **Résolution :** La logique de la commande de menu contextuel a été entièrement refactorisée pour devenir dynamique. Au lieu d'utiliser des valeurs fixes, le client Discord appelle désormais la méthode `tools/list` du serveur MCP pour récupérer la définition à jour de l'outil. Il extrait ensuite les options valides (`enum`) des paramètres `description_type` et `language` et les utilise pour construire dynamiquement les menus de sélection de l'interface utilisateur. Cette correction rend le client automatiquement compatible avec toute modification future du schéma de l'outil sur le serveur MCP.
    *   **Statut :** RÉSOLU.

*   **Erreur de Parsing de l'Agent `Parameter Extractor`**
    *   **Analyse :** Le serveur levait une exception `NameError: name 'ParameterExtractionResult' is not defined` dans `agent_orchestrator.py` malgré la présence de l'importation correcte en haut du fichier. L'enquête a révélé que la cause sous-jacente était une dépendance circulaire entre les modules, qui empêchait l'initialisation complète du nom au moment du chargement du module.
    *   **Résolution :** L'importation `from app.schemas.chat_schemas import ParameterExtractorResult` a été déplacée du niveau du module (global) vers l'intérieur de la fonction `process_user_message`, juste avant son utilisation. Cet import local (ou différé) contourne le problème de dépendance circulaire en garantissant que le nom est résolu au moment de l'exécution plutôt qu'au moment du chargement initial.
    *   **Statut :** RÉSOLU.
