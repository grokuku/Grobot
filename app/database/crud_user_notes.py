...
# app/database/crud_user_notes.py
    ...
    from app.database import sql_models
    from app.schemas import user_note_schemas, chat_schemas
    from app.database.crud_user_profiles import get_or_create_user_profile
    ...
    def create_user_notes_from_archivist(
        db: Session, 
        bot_id: int, 
        discord_user_id: str, 
        notes_to_create: List[chat_schemas.ArchivistNote]
    ) -> None:
        """
        Saves a list of notes from the Archivist agent to the database.
    
        It ensures a user profile exists for the given user and bot before creating the notes.
    
        Args:
            db: The database session.
            bot_id: The ID of the bot context.
            discord_user_id: The Discord ID of the user the notes are about.
            notes_to_create: A list of note objects from the Archivist's decision.
        """
        # For now, we assume notes are created in the context of a server, not a PM.
        # We'll use the user ID as a placeholder for the server ID, as the profile
        # is primarily user-centric. This may need refinement if server-specific
        # profiles become more distinct.
        server_id_placeholder = "user_centric"
    
        user_profile = get_or_create_user_profile(
            db=db,
            bot_id=bot_id,
            user_id=discord_user_id,
            server_id=server_id_placeholder # Placeholder as context is user-focused
        )
    
        for note_data in notes_to_create:
            note_schema = user_note_schemas.UserNoteCreate(
                user_profile_id=user_profile.id,
                content=note_data.fact,
                reliability_score=note_data.reliability_score
            )
            create_user_note(db=db, note=note_schema)